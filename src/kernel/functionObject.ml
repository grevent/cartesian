...
exception NoPatternForFunctionEval
...
object
  ...
  method apply (env: 'expression Env.env) (lst: 'expression list) = 
	...
	[] -> 
	  raise NoPatternForFunctionEval
      | (patterns,expr)::suite -> 
	try
	  let bools = List.map2 (fun pattern expr -> pattern#matchToExpression env expr) patterns lst in
